...
import { Chess } from "chess.js"; // Import Chess.js for pre-computation
...
router.post("/", (async (req: Request, res: Response) => {
  ...
  const enrichedMoves = movesToExplain.map((moveObj) => {
    ...
    const pieceData = gameInstance?.get(startSquare as any); // Cast to 'any' if Square type causes issues with chess.js .get()
    ...
    const pieceColor = pieceData
      ? pieceData.color === "w"
        ...
        : "Black"
      : "Unknown Color";
    return { ...moveObj, startSquare, endSquare, pieceType, pieceColor };
  });
  // --- End Pre-computation ---

  // --- Cache Lookup ---
  const cacheCutoffDate = subDays(new Date(), CACHE_DURATION_DAYS);
  try {
    ...
    if (cachedAnalysis?.llm_explanation) {
  ...
  if (checkmatingMove) {
    ...
    const otherMovesFormat = otherMoves
      .map(
        (m) =>
          `*   **${m.move}:** The ${m.pieceColor} ${m.pieceType} on ${m.startSquare} moves to ${m.endSquare}. [1 sentence idea], but this is **inferior** as it doesn't deliver checkmate.`
      )
      .join("\n");
...
*   No conversational filler. Adhere precisely to the specified output format.`;
    ...
    const enrichedMoveListInData = enrichedMoves
      .map(
        (m, i) =>
          `    ${i + 1}. ${m.move} (This is the ${m.pieceColor} ${
            m.pieceType
          } moving from ${m.startSquare} to ${m.endSquare})`
      )
      .join("\n");
    ...
    try {
      ...
      await prisma.analysis.create({
        data: {
          ...
          stockfish_best_moves: movesToCache as any,
        },
      });
    } catch (dbError) {
