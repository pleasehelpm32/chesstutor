...
exports.stockfishManager = void 0;
// src/utils/stockfishManager.ts
const path_1 = __importDefault(require("path"));
const os_1 = __importDefault(require("os"));
const child_process_1 = require("child_process");
// --- Configuration ---
function getStockfishExecutableName() {
    ...
    if (platform === "darwin") {
        ...
        console.warn(`Unsupported platform: ${platform}, using default 'stockfish'`);
...
const stockfishPath = process.env.STOCKFISH_PATH ||
    path_1.default.join(__dirname, "../../bin", stockfishExecutable); // Path from dist/utils to project_root/bin
// --- State ---
...
function initializeStockfish() {
    ...
    initializationPromise = new Promise((resolve, reject) => {
        ...
        if (engineProcess) {
            console.log("Stockfish process exists but may not be ready. Attempting to kill and restart.");
            engineProcess.kill();
            engineProcess = null;
        }
        isEngineReady = false;
        ...
        try {
            ...
            engineProcess = (0, child_process_1.spawn)(stockfishPath, [], {
            ...
            engineProcess.stdin.on("error", (err) => {
                ...
                if (rejectInitialization) {
                    rejectInitialization(new Error(`Stockfish stdin error: ${err.message}`));
            ...
            engineProcess.stdin.on("close", () => {
                ...
                if (!isEngineReady && rejectInitialization) {
                    rejectInitialization(new Error("Stockfish stdin stream closed prematurely."));
            ...
            engineProcess.on("error", (err) => {
                console.error("!!! Failed to spawn Stockfish process (on 'error') !!!", err);
                if (rejectInitialization) {
                    ...
                    if (line) {
                        ...
                        if (line === "uciok") {
                            ...
                            if (!sentIsReady && rejectInitialization) {
                                const err = new Error("Failed to send 'isready' command after 'uciok'.");
            ...
            engineProcess.on("close", (code) => {
                ...
                if (!isEngineReady && rejectInitialization) {
                    ...
                    rejectInitialization(new Error(`Stockfish process exited prematurely with code ${code}.`));
                    resolveInitialization = null;
                    rejectInitialization = null;
                }
                engineProcess = null;
            ...
            if ((_a = engineProcess === null || engineProcess === void 0 ? void 0 : engineProcess.stdin) === null || _a === void 0 ? void 0 : _a.writable) {
                sendCommand("uci");
                console.log("Stockfish process spawned, sent 'uci', waiting for uciok/readyok...");
            }
            else {
    ...
    const initTimeout = setTimeout(() => {
        if (!isEngineReady && rejectInitialization) {
            console.error(`Stockfish initialization timed out after ${INITIALIZATION_TIMEOUT_MS}ms.`);
...
function sendCommand(command) {
    ...
    const isStdinWritable = !!((_a = engineProcess === null || engineProcess === void 0 ? void 0 : engineProcess.stdin) === null || _a === void 0 ? void 0 : _a.writable);
    const isProcessKilled = !!(engineProcess === null || engineProcess === void 0 ? void 0 : engineProcess.killed);
    if (processExists && isStdinWritable && !isProcessKilled) {
        ...
        console.error(`Cannot send command "${command}": ProcessExists=${processExists}, StdinWritable=${isStdinWritable}, ProcessKilled=${isProcessKilled}`);
        return false;
    }
}
function isReady() {
    ...
    return new Promise((resolve) => {
        if (engineProcess && !engineProcess.killed) {
            ...
            currentProcess.on("close", (code) => {
                console.log(`Stockfish process exited (during shutdown) with code ${code}.`);
